...
gameContainer.addEventListener("mousemove", (e) => {
  let locationleft = e.clientX - gameContainer.getBoundingClientRect().left;
  let locationright = e.clientX - gameContainer.getBoundingClientRect().right;
  if (locationleft > 0 && locationright < 0) {
...
function createBoom() {
  ...
  setTimeout(createBoom, state.speed == 0 ? 7000 : 7000 / state.speed);
}
...
let handleTime = setInterval(() => {
  if (gameover == false) {
    ...
    if (check(time / 10)) {
      state.speed = state.speed + 0.7;
    }
    time = time + 1;
    document.querySelector(".time").innerHTML = time;
  }
}, 1000);
let handleMainEgg = setInterval(
  function create() {
    const egg = document.createElement("img");
    egg.classList.add("egg");
    egg.src = "./chanh.png";
  ...
  state.speed == 0 ? 10000 : 10000 / state.speed
);
let handleMainBoom = setInterval(
  function create() {
    const boom = document.createElement("img");
    boom.classList.add("boom");
    boom.src = "./boom.png";
  ...
  state.speed == 0 ? 14000 : 14000 / state.speed
);
createEgg();
createBoom();
